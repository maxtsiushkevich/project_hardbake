...
from api.schemas.packet_processor import PcapProcessResult, UploadStatus, StreamSummary
...
class PcapRedisRepository:
    ...
    async def save(self, data: PcapProcessResult, upload_id: UUID):
        key_status = f"{upload_id}:status"
        key_streams = f"{upload_id}:streams"

        await self.connection.set(key_status, pickle.dumps(data.info))
        await self.connection.set(key_streams, pickle.dumps(StreamSummary(
            tcp_streams=data.tcp_streams,
            udp_streams=data.udp_streams
        )))
    ...
    async def get_streams(self, upload_id: UUID):
        key_streams = f"{upload_id}:streams"
